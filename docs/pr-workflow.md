...
```
        ...
        └───────────────────────┬─────────────────────┘
                                 ▼
        ┌─────────────────────────────────────────────┐
   ┌───▶│  REVIEWER: review the diff → inline comments  │
   │    └───────────────────────┬─────────────────────┘
   │                findings?    │
   │            ┌── yes ─────────┤── no ──┐
   │            ▼                          ▼
...
The local gate this project's dev role runs before pushing fixes is:

```
cd backend && pytest -q && cd ../frontend && node --test tests/*.test.mjs
```
