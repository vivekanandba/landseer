...
4. **Review loop** — repeat, up to 5 rounds:
   1. **Reviewer role.** Review the PR diff thoroughly and post findings as
      inline PR comments. Use `/code-review high --comment` if your setup has
      it; otherwise review the diff and post comments with `gh` (or the
      `review` skill).
   2. If the review reports **no findings**, exit the loop.
   ...
   4. Run the full local gate: `cd backend && pytest -q && cd ../frontend && node --test tests/*.test.mjs`.
